...
const texts = ["HACIMI PANTEQQ"];
let currentText = texts[Math.floor(Math.random() * texts.length)];
let letterIndex = 0;

function getRandomLetter() {
	const letter = currentText.charAt(letterIndex);
	letterIndex = (letterIndex + 1) % currentText.length;
	if (letterIndex === 0) {
		// Switch text after completing one cycle
